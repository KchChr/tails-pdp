...
nicht privilegierter Teil führt 59 automatisierte Rust-Tests in fünf Testbereichen aus. Die Option
...
Updaters nach einem fehlgeschlagenen Commit ab. Zwei
...
Release-Build über \texttt{sudo} an und ruft \path{test-e2e.sh} auf. Dieses Skript bildet den
gemeinsamen Einstiegspunkt der End-to-End-Stufe und
ruft für die Test-IDs \texttt{E2E-01} bis \texttt{E2E-17} jeweils eine eigene Bashdatei unter
...
Die Szenarien \texttt{E2E-11} bis \texttt{E2E-17} werden darin anschließend über
\path{test-evaluation.sh} mit jeweils eigener Runtime und isoliertem Arbeitsverzeichnis
...
Nach Abschluss aller 17 End-to-End-Szenarien ruft \path{test.sh} denselben Evaluationsrunner ein
...
\texttt{E2E-11} bis \texttt{E2E-17} doppelt zu starten. Die Teilrunner bleiben für gezielte
...
Deskriptoren mit \texttt{EBADF} ungültig werden.
...
Die nachfolgend dargestellten Ergebnisse wurden am 10.~September 2026 auf dem in
Abschnitt~6.1 beschriebenen Zielsystem erhoben. Die funktionalen Abschlussprüfungen beziehen sich
auf den Quellstand \texttt{35e1d2b}; die quantitativen Messwerte wurden zuvor mit dem Quellstand
\texttt{9848f19} auf demselben System erfasst. Zwischen beiden Ständen wurde die Behandlung einer
überschrittenen Attributkapazität korrigiert. Da diese Änderung den Updatepfad des
Attributloaders betrifft, nicht jedoch den eBPF-Pfad einer Dateiöffnung, werden die vorhandenen
Messwerte weiterhin zur Charakterisierung des Prototyps herangezogen. Insbesondere die gemessene
Attributreaktionszeit ist dennoch als Wert des früheren Quellstands auszuweisen und nicht als
Neuvermessung der Korrektur zu verstehen.

\subsubsection{Ergebnisse der funktionalen Prüfungen}

Alle 59 Unit- und Komponententests wurden erfolgreich ausgeführt. Davon entfallen 16 Tests auf
die gemeinsame Policylogik, 14 auf den Policyloader, 13 auf den Attributloader, zwei auf den
...
ebenfalls erfolgreich. Damit ist nachgewiesen, dass der untersuchte Quellstand übersetzbar ist
und die in Abschnitt~6.2.2 beschriebenen Assertions erfüllt.

Die vollständige Ausführung von \path{test.sh} endet gleichwohl nicht mit einem Gesamterfolg.
Nach den bestandenen Rust-Tests meldet Clippy für
\path{tails-pdp-userspace-common/src/lib.rs} den Befund
\texttt{items\_after\_test\_module}. Die separat geprüften, im Zuge der Kapazitätskorrektur
geänderten Crates bestanden die statische Analyse mit als Fehler behandelten Warnungen. Der
verbleibende Befund hat keinen beobachteten Einfluss auf das Laufzeitverhalten, zeigt jedoch,
dass die definierte Qualitätsprüfkette des gesamten Workspace noch nicht vollständig erfolgreich
ist.

Auf dem Zielkernel bestanden alle 17 End-to-End-Szenarien. Die Runtime wurde vom
...
geschlossen, während die erlaubten Deskriptoren desselben Prozesses geöffnet blieben. War ein
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.28\textwidth}p{0.18\textwidth}p{0.45\textwidth}}
        ...
        \hline
        Rust-Tests & 59 von 59 bestanden & Unit- und Komponentennachweise ohne realen
        Kernelpfad \\
        End-to-End-Tests & 17 von 17 bestanden & Vollständiger Pfad einschließlich
        Kernel-Verifier, LSM-Attach und Enforcement \\
        Formatprüfung & Bestanden & Keine Formatabweichungen im Rust-Code \\
        Clippy-Gesamtprüfung & Nicht bestanden & Bestehender Strukturhinweis
        \texttt{items\_after\_test\_module} \\
...
einen General-Protection-Fault. Der Abschlusslauf benötigte rund 178~Sekunden. Dieses Ergebnis
...
Im nebenläufigen Stresstest wurden während zehn Policyzyklen 2535 Wechsel der Ressource hinter
...
20\,000 Dateiöffnungen mit warmem Cache gemessen. Der Median stieg von 3,845~$\mu$s ohne
Runtime auf 4,389~$\mu$s bei aktiver Runtime mit leerem Policystand und auf
4,412~$\mu$s bei passender Permit-Policy. Dies entspricht einem medianen Mehraufwand von
14,1 beziehungsweise 14,7~Prozent. Die Messung umfasst \texttt{os.open()} sowie den Aufwand der
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.40\textwidth}p{0.13\textwidth}p{0.17\textwidth}p{0.17\textwidth}}
        ...
        \hline
        Dateiöffnung ohne Runtime & 20\,000 & 3,845~$\mu$s & 4,024~$\mu$s \\
        Dateiöffnung, Runtime ohne Policy & 20\,000 & 4,389~$\mu$s & 4,621~$\mu$s \\
        Dateiöffnung, passende Permit-Policy & 20\,000 & 4,412~$\mu$s & 6,629~$\mu$s \\
        Policyänderung bis neues Deny & 10 & 102,41~ms & 103,51~ms \\
        Attributänderung bis neues Deny & 10 & 103,34~ms & 114,12~ms \\
        Policyänderung bis \texttt{EBADF} & 10 & 58,91~ms & 110,65~ms \\
        \hline
    \end{tabular}
\end{table}

Die Reaktionszeit vom atomaren Austausch einer Datei bis zur beobachtbaren neuen Entscheidung
lag im Median bei 102,41~ms für Policyänderungen und 103,34~ms für Attributänderungen. Dabei wurde
...
beobachteten \texttt{EBADF} im Median 58,91~ms. Zusätzlich wurde die Aktivierung durch zwei
aufeinanderfolgende Abfragen der Policygeneration zeitlich eingegrenzt. Über zehn Wiederholungen
lagen die unteren Grenzen der Aktivierung-bis-Entzug-Intervalle zwischen 7,02 und 8,45~ms, die
oberen Grenzen zwischen 9,34 und 10,84~ms. Eine scheinexakte Einzelzeit wird daraus nicht
abgeleitet. Bei nur zehn Wiederholungen entspricht das angegebene p95 jeweils dem beobachteten
...
die getesteten Policyformen, Attributtypen und den Hook \texttt{file\_open}. Insbesondere folgt
...
zusätzliche Aufwand und die Reaktions- und Entzugslatenz quantitativ untersucht wurden; mangels
vorgegebenem Grenzwert ist damit keine Aussage verbunden, ob die gemessenen Werte für einen
bestimmten Produktiveinsatz akzeptabel sind. OA-04 wird durch die archivierten Konfigurationen,
