...
Der LSM-Hook \texttt{file\_open} stellt dem Programm einen Zeiger auf das zu öffnende Kernelobjekt \texttt{struct file} bereit; die UID wird über den aktuellen eBPF-Kontext ermittelt \citep{kernelLSMdevelopment}. Als Subjektidentität legt der Prototyp ausdrücklich die \emph{Real UID} des aufrufenden Prozesses fest. Ayas \texttt{ctx.uid()} basiert auf dem BPF-Helper \texttt{bpf\_get\_current\_uid\_gid}; dieser liefert die im Kernel als Real UID geführte Identität \citep{kernelCredentials}. Effektive, Saved-Set- und Filesystem-UID sind damit keine Eingaben der Policyentscheidung. BPF-LSM ergänzt die Hook-Argumente um den Rückgabewert des zuvor ausgeführten BPF-LSM-Programms oder um null beim ersten Programm \citep{kernelBpfLsm}. Die Verkettung allein bewahrt eine frühere Ablehnung nicht automatisch, wenn ein nachfolgendes Programm den Wert ignoriert. Das Einstiegspunktprogramm prüft ihn deshalb explizit und gibt einen Wert ungleich null unverändert zurück.
