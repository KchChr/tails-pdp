...
\begin{itemize}
    ...
    Der Userspace-PEP rekonstruiert nach entscheidungsrelevanten Ereignissen bestehende Dateizugriffe, bewertet sie erneut und versucht, erkannte Policy-Verletzungen durch das Schließen des betroffenen FDs durchzusetzen.
...
Neue Policy-Generationen werden vollständig in der inaktiven Bank vorbereitet und erst anschließend aktiviert.
Damit verhindert das Verfahren die vorzeitige Aktivierung unvollständig geschriebener Generationen.
Eine unveränderte Lesesicht über mehrere aufeinanderfolgende Generationenwechsel hinweg ist durch das Zwei-Bank-Verfahren allein jedoch nicht garantiert.
...
Im Prototyp betrifft dies die aktuelle Zeit sowie vom Nutzer frei wählbare Attribute (vgl. FA-10).

Der Attributloader schreibt die Unix-Zeit einmal pro Sekunde in die gepinnte Zeit-Map
...
Verzeichnis \texttt{attributes/} rekursiv. Da ein Änderungsvorgang mehrere unmittelbar
aufeinanderfolgende \texttt{inotify}-Ereignisse erzeugen kann \citep{manInotify}, wartet der Loader
nach dem ersten Ereignis 100 Millisekunden, bevor er den Gesamtbestand erneut einliest und
validiert. Diese prototypspezifisch gewählte Bündelungsverzögerung reduziert redundante
Verarbeitungsvorgänge und die Wahrscheinlichkeit, einen vorübergehenden Zwischenstand einzulesen.
Erst nach erfolgreicher Validierung und Übernahme wird die neue Attributgeneration aktiviert. Ohne
Dateisystemereignis findet keine periodische Neuladung dieser Dateien statt.
...
Wird eine Verletzung erkannt, versucht der Userspace-PEP, den konkret betroffenen FD zu schließen.

Die Nachbewertung liest die Entscheidungsgrundlagen aus den gepinnten Maps; sie erhält keine fertigen Entscheidungen vom Kernelspace-PEP.

\subsection{Komponenten}
Die folgenden Komponenten übernehmen die Zuständigkeiten innerhalb der beschriebenen Datenflüsse.
...
Die Policyverwaltung verantwortet außerdem die Vorbereitung und Aktivierung neuer Policy-Generationen. Das zugehörige Verfahren wird in Abschnitt~\ref{sec:entscheidung-policy-generationen} erläutert.
...
Der Userspace-PEP bewertet laufende Prozesse und deren verwendete Ressourcen nur nach erfolgreichen entscheidungsrelevanten Aktivierungen oder zu Zeitpunkten, an denen sich das Ergebnis einer aktiven zeitabhängigen Policybedingung ändern kann.
Dazu nutzt er den Prozess- und Ressourcenstatus des Betriebssystems, beispielsweise Informationen über offene FDs.
Diese Informationen werden gegen die aktuell aktiven Policies geprüft. 
Wird eine Verletzung erkannt, versucht der Userspace-PEP, den betroffenen FD zu schließen.
...
Die zentralen Datenobjekte werden nach ihrer Verwendung im Userspace und Kernelspace unterschieden.
...
Dazu zählen laufende Prozesse, Benutzer-IDs, Prozessnamen, offene FDs und geöffnete Dateien.
...
Sie liegen als feste Einträge in eBPF-Maps vor.
...
\subsubsection{Trennung von Userspace und Kernelspace}\label{sec:entscheidung-userspace-kernelspace}
...
\cite[Abschnitt~3.4]{anderson1972computer}.
Wright et al. beschreiben, dass LSM-Hooks unmittelbar vor dem Zugriff auf interne Kernelobjekte ausgeführt werden und den Zugriff erlauben oder verweigern können~\cite{wright2002linux}.
Für den in dieser Arbeit gewählten LSM-Durchsetzungspfad wird die Zugriffskontrollentscheidung daher innerhalb des Kernel-Ausführungspfades getroffen.
...
Gleichzeitig entsteht nach einer erfolgreichen Dateiöffnung ein FD, über den der Zugriff weiterbestehen kann.
...
Andauernde Zugriffssituationen können über geöffnete FDs beobachtet werden.
Dafür stellt Linux mit \texttt{/proc/<PID>/fd} eine Schnittstelle bereit, über die geöffnete FDs eines Prozesses nachvollzogen werden können~\citep{manProcPidFd}.
...
In diesem Fall könnte eine Zugriffentscheidung auf einer widersprüchlichen Mischung aus Einträgen der bisherigen und der neu vorbereiteten Policy-Generation beruhen.
...
erlaubt. Die XACML-Ergebnisse \textit{Indeterminate} und \textit{NotApplicable} werden dabei nicht
als eigene Ergebnisse abgebildet.
...
Wenn der Zugriff erlaubt wurde, kann der Prozess die Datei anschließend über den erhaltenen FD weiter verwenden.
...
Der Userspace-PEP liest Informationen über laufende Prozesse und offene FDs aus dem System, wenn eine erfolgreiche entscheidungsrelevante Aktivierung oder eine relevante Zeitgrenze dies auslöst.
...
Wird eine Verletzung erkannt, versucht der Userspace-PEP, den zugeordneten FD zu schließen.
...
Ein gefundener FD repräsentiert dabei eine fortdauernde Nutzungssituation, jedoch keine registrierte ASBAC-Subscription.
...
Diese Komponente arbeitet ereignisgetrieben und nicht als Ersatz für den LSM-Hook. Die
Neubewertung und der Entzugsversuch bilden keine atomare Operation: Zwischen dem Einlesen der
Prozessinformationen, der Policy-Auswertung und dem Schließen des FDs kann sich dessen
Zuordnung verändern. Zudem beendet das Schließen eines einzelnen Descriptors weder bereits
laufende E/A-Operationen noch Zugriffe über duplizierte oder vererbte FDs oder
bestehende Speicherabbildungen (vgl. Abschnitt~\ref{sec:fd-monitoring-foundations}). Der
Userspace-PEP kann einen erkannten Verstoß daher nur nachträglich und bestmöglich behandeln; ein
vollständiger oder verzögerungsfreier Entzug ist nicht garantiert.
...
Bei Dateizugriffen wird der fortdauernde Zugriff über einen FD repräsentiert.
Dieser FD ist daher der konkrete Ansatzpunkt, um genau den betroffenen Zugriff zu entziehen.

Die Entscheidung lautet, Enforcement durch das Schließen des betroffenen FDs umzusetzen.
Der Userspace-PEP ermittelt Prozess-ID und File-Descriptor-Nummer der verletzenden Zugriffssituation.
Anschließend schließt die Enforcement-Komponente genau diesen FD im betroffenen Prozess.
Im aktuellen Prototyp geschieht dies auf x86\_64-Linux über \texttt{ptrace}, indem im Zielprozess ein \texttt{close}-Systemaufruf für den ausgewählten FD ausgeführt wird.

Diese Entscheidung ist feingranularer als ein Prozessabbruch.
Wenn ein Prozess mehrere Dateien geöffnet hat, kann nur der FD geschlossen werden, der zu dem unzulässig gewordenen Zugriff gehört.
Andere FDs desselben Prozesses bleiben bestehen, sofern sie nicht selbst gegen eine Policy verstoßen.
Damit passt die Maßnahme zu der Anforderung, nur den betroffenen Zugriff zu beenden.

Eine Alternative wäre, den gesamten Prozess zu beenden.
...
Zudem ist das Schließen fremder FDs ein invasiver Eingriff, der sorgfältig auf den tatsächlich betroffenen FD begrenzt werden muss.
