...
Die Anforderungen ergeben sich aus der Zielsetzung, zu untersuchen, wie ASBAC im Linux-Kernel prototypisch umgesetzt werden kann.
Sie beschreiben das Verhalten, das der Prototyp im Rahmen der Arbeit zeigen soll.

Für den Prototyp wird ein vertrauenswürdig administriertes Linux-Zielsystem vorausgesetzt. Der Administrator und der Kernel gehören zur Vertrauensbasis. Der Administrator darf Policies, Attributquellen, eBPF-Maps und Programme verändern sowie die Runtime beenden; Schutz gegen solche administrativen Eingriffe ist nicht Gegenstand der Anforderungen. Die Integrität der Policy- und Attributquellen sowie der bereitgestellten Zeitinformationen wird vorausgesetzt und ist durch die Administration sicherzustellen. Vorgesehen ist der Betrieb einer einzelnen Runtime; eine Absicherung gegen konkurrierende administrative Schreibzugriffe wird nicht gefordert.

Die zusätzliche Zugriffskontrolle des Prototyps wird nur nach erfolgreicher Initialisierung und solange das Hauptprogramm läuft bereitgestellt. Eine Fortsetzung dieses Schutzes nach Beendigung oder Ausfall der Runtime ist nicht Bestandteil des Prototypumfangs.
...
\begin{description}
    ...
    \item[FA-03 Zugriffskontrolle bei Dateiöffnungen:]
    Der Prototyp muss Dateiöffnungen über den LSM-Hook \texttt{file\_open} abfangen können.
    ...
    \item[FA-04 Entscheidungsfindung anhand von Policies:]
    ...
    \item[FA-05 Unterstützung dynamischer Attribute:]
    ...
    \item[FA-06 Überwachung und Enforcement bestehender Dateizugriffe:]
    Der Prototyp muss bereits bestehende Dateizugriffe überwachen und versuchen, sie durch Schließen des betroffenen FDs zu beenden, wenn sie durch entscheidungsrelevante Policy- oder Attributänderungen unzulässig werden.
    Als bestehender Zugriff gilt im Rahmen des Prototyps ein geöffneter FD eines laufenden Prozesses auf eine durch eine Policy erfasste Datei.
    Der Userspace-PEP ermittelt den zugehörigen Prozess und FD, bewertet den Zugriff anhand der aktuellen Policy- und Attributlage und führt bei einer erkannten Verletzung einen gezielten Schließversuch im Zielprozess aus.
    Im Rahmen des Prototyps wird dieser Entzug als Best-Effort-Maßnahme verstanden: Der Schließversuch ist erforderlich, sein Erfolg kann insbesondere bei einem nicht erreichbaren Zielprozess jedoch nicht garantiert werden. Ein fehlgeschlagener Versuch muss protokolliert werden und darf die Behandlung weiterer festgestellter Verletzungen nicht verhindern.
    Eine reine Ausgabe der Verletzung ohne Schließversuch genügt der Anforderung nicht.

    \textit{Akzeptanzkriterium:} Öffnet ein Prozess eine Datei zunächst zulässig und wird dieser Zugriff anschließend durch eine Policy- oder Attributänderung unzulässig, führt der Userspace-PEP bei der Nachbewertung einen gezielten Schließversuch für den betroffenen FD aus. In einem kontrollierten Erfolgsfall wird der Deskriptor tatsächlich geschlossen; andere FDs desselben Prozesses bleiben geöffnet, sofern sie nicht selbst eine Policy verletzen. In einem kontrollierten Fehlerfall mit nicht erreichbarem Zielprozess wird der fehlgeschlagene Versuch protokolliert und die Behandlung weiterer festgestellter Verletzungen fortgesetzt. Ein fehlgeschlagener Schließversuch wird dabei nicht als erfolgreicher Entzug gewertet.

    \item[FA-07 Administrationsschnittstelle:]
    ...
    \item[FA-08 Gültige Policy-Generationen:]
    ...
    \item[FA-09 Validierung von Policies:]
    Der Prototyp muss eingelesene Policies vor ihrer Übersetzung in die Kernelrepräsentation syntaktisch und semantisch validieren.
    Policies, die nicht dem unterstützten Policyformat entsprechen oder nicht in die vorgesehene Kernelrepräsentation überführt werden können, dürfen nicht für Zugriffsentscheidungen aktiviert werden.
    
    \textit{Akzeptanzkriterium:} Eine syntaktisch oder semantisch ungültige Policy wird mit einer nachvollziehbaren Fehlermeldung abgelehnt und nicht in die aktiven Policy-Maps übernommen. Die bislang aktive, gültige Policy-Generation bleibt unverändert.

    \item[FA-10 Beliebige Attributnamen:]
    ...
    Zulässige Attributnamen dürfen nicht leer sein und bestehen aus American Standard Code for Information Interchange (ASCII)-Buchstaben (\texttt{A-Z}, \texttt{a-z}), Ziffern (\texttt{0-9}), Unterstrich (\texttt{\_}) und Bindestrich (\texttt{-}).
...
\begin{description}
    ...
    Der Prototyp darf die Stabilität des Linux-Kernels nicht gefährden.
    ...
    \textit{Akzeptanzkriterium:} Die Auswirkungen auf die Ausführungszeit werden in der Evaluation betrachtet und bewertet.

    \item[OA-04 Reproduzierbarkeit:]
    Die Evaluation des Prototyps soll unter dokumentierten Bedingungen wiederholbar sein.
    
    \textit{Akzeptanzkriterium:} Testumgebung, Policies und Testszenarien werden so beschrieben, dass die Ergebnisse nachvollzogen und wiederholt< werden können.
\end{description}
