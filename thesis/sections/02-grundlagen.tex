...
Für die Umsetzung im Linux-Kernel sind außerdem die Trennung zwischen Userspace und Kernelspace, LSM sowie eBPF relevant.
...
Abschließend werden die für den Prototyp spezifischen Grundlagen betrachtet: die Verwendung von Aya als Rust-Framework, die Abbildung dynamischer Attribute und Policy-Generationen sowie die Überwachung bereits bestehender Zugriffe über FD.
...
Eine Zugriffsentscheidung basiert auf einer Menge von Policies, die festlegen, unter welchen Bedingungen eine Operation erlaubt oder verweigert wird.
...
Der Reference Monitor ist eine abstrakte Sicherheitskomponente, die jeden Zugriff eines Subjekts auf eine geschützte Ressource überprüft, bevor dieser Zugriff ausgeführt wird.
Damit ein Reference Monitor wirksam ist, muss er insbesondere drei Eigenschaften erfüllen: Er muss bei jedem relevanten Zugriff durchlaufen werden, gegen Manipulation geschützt sein und hinreichend klein beziehungsweise nachvollziehbar sein, damit seine Korrektheit überprüft werden kann \citep[Abschnitt~3.4, S.~17]{anderson1972computer}.
...
Dies steht im Zusammenhang mit dem Prinzip der vollständigen Vermittlung, nach dem jeder Zugriff auf eine geschütztee Ressource vor seiner Ausführung geprüft werden muss \citep{saltzer1975protection}.

Im Kontext dieser Arbeit bildet das Reference-Monitor-Konzept die Grundlage für die Frage, wie ASBAC im Linux-Kernel umgesetzt werden kann.
...
Attribute-Based Access Control (ABAC) ist ein Zugriffskontrollmodell, bei dem eine Autorisierungsentscheidung nicht allein anhand einer festen Subjektidentität oder einer an der Ressource hinterlegten Access Control List (ACL) getroffen wird. Eine ACL ordnet einzelnen Subjekten oder Gruppen konkrete Berechtigungen für eine Ressource zu. Ebenso beruht die Entscheidung nicht ausschließlich auf einer zugewiesenen Rolle, wie bei Role-Based Access Control (RBAC).
...
ASBAC erweitert attributbasierte Zugriffskontrolle um fortlaufend veränderliche Entscheidungsgrundlagen und Autorisierungsentscheidungen.
...
Heutelbeck beschreibt ASBAC im Zusammenhang mit der SAPL als Publish-Subscribe-Modell.
Ein Policy Enforcement Point stellt dazu eine Autorisierungsanfrage als Subscription an einen PDP.
Der PDP bezieht die für die Entscheidung benötigten Datenströme ein und kann dem PEP aktualisierte Entscheidungen übermitteln, wenn sich Policies, Attribute oder weitere Entscheidungsgrundlagen ändern \citep{heutelbeck2021asbac}.
...
Usage Control dient in dieser Arbeit als verwandtes theoretisches Konzept, ersetzt jedoch nicht die Definition von ASBAC.
...
Ein geöffneter FD wird dabei als technische Repräsentation einer fortdauernden Nutzungssituation betrachtet, nicht als ASBAC-Subscription.
...
EXtensible Access Control Markup Language (XACML) beschreibt beispielsweise eine Architektur mit PDP und PEP sowie eine Policy-Sprache zur Beschreibung attributbasierter Entscheidungen \citep{oasisXacml30}.
Der PDP wertet die für eine Zugriffsanfrage anwendbaren Policies aus und erzeugt eine Autorisierungsentscheidung.
Der PEP schützt die Ressource, bildet eine Entscheidungsanfrage und setzt die erhaltene Entscheidung technisch durch.
...
Entsprechend rekonstruiert die Userspace-Komponente bestehende Zugriffssituationen aus Prozess- und File-Descriptor-Informationen, wertet sie lokal anhand der aktiven Policies und Attribute aus und versucht bei einer Deny-Entscheidung, den betroffenen FD zu schließen.
Auch diese Komponente verbindet somit eine Policy-Entscheidungsfunktion mit einer Durchsetzungsfunktion.
In dieser Arbeit werden die Bezeichnungen \emph{Kernelspace-PEP} und \emph{Userspace-PEP} verwendet, um die jeweilige Durchsetzungsfunktion hervorzuheben.
...
\subsection{Linux Security Modules und Linux Security Module-Hooks}
LSM stellen ein Framework bereit, über das Sicherheitsmodule an sicherheitsrelevanten Stellen des Kernels Entscheidungen treffen können.
...
Für den Prototyp ist der LSM-Hook \texttt{file\_open} maßgeblich. Er wird beim Öffnen einer Datei aufgerufen und deckt damit die in dieser Arbeit betrachtete Operation ab. Seine Entscheidung bezieht sich jedoch ausschließlich auf den aktuellen Öffnungsvorgang. Ändern sich Attribute erst danach, wird ein bereits geöffneter Dateizugriff nicht erneut durch denselben Hook bewertet. Daher ist für solche bestehenden Zugriffssituationen eine ergänzende nachträgliche Überwachung erforderlich.

\subsection{extended Berkeley Packet Filter und extended Berkeley Packet Filter-Linux Security Module}
Der eBPF ist ein Mechanismus, mit dem Programme in einer eingeschränkten Ausführungsumgebung im Linux-Kernel ausgeführt werden können.
...
In dieser Arbeit wird deshalb eBPF verwendet, wenn der moderne Mechanismus für Programme, Maps und Verifier (vgl. Abschnitt~\ref{sec:ebpf-verifier}) gemeint ist.
...
Die Kernel-Dokumentation beschreibt Maps als generischen Speicher für unterschiedliche Datentypen, der unter anderem den Datenaustausch zwischen Kernelspace und Userspace ermöglicht \citep{kernelBpfMaps,manBpf}.
...
\subsection{eBPF-Verifier, Stack und Programmkomplexität}\label{sec:ebpf-verifier}
...
Auch der Zugriff auf Kernelinformationen erfolgt nicht beliebig, sondern über den Hook-Kontext, Maps oder für den jeweiligen Programmtyp zugelassene Hilfsfunktionen \citep{kernelBpfVerifier}.
...
Der Stack eignet sich damit nur für kleine lokale Hilfsdaten, etwa Schlüssel für den Zugriff auf Map-Einträge oder wenige temporäre Werte.
...
\subsection{Aya als Rust-Framework für eBPF}\label{sec:aya}
...
Neben Rust-basierten Frameworks existieren etablierte eBPF-Werkzeuge mit C- sowie Hochsprachen-Schnittstellen.
...
Die Idee veränderlicher Attribute knüpft an Usage-Control-Modelle an, in denen Attributmutabilität als ausdrückliches Merkmal betrachtet wird \citep[S.~52]{gouglidis2018usecon}.
...
eBPF-Programme können Map-Einträge während ihrer Ausführung lesen, während Userspace-Anwendungen dieselben Maps aktualisieren können \citep{manBpf}.
Der Prototyp nutzt diese Eigenschaft, um aktuelle Attributwerte für Zugriffsentscheidungen bereitzustellen.
...
\subsection{Überwachung bestehender Zugriffe und File Descriptors}\label{sec:fd-monitoring-foundations}
Ein Dateizugriff in Linux wird häufig über einen FD repräsentiert.
Der Systemaufruf \texttt{open()} liefert bei Erfolg einen nichtnegativen FD zurück, der in nachfolgenden Systemaufrufen wie \texttt{read()}, \texttt{write()} oder \texttt{lseek()} als Referenz auf die geöffnete Datei verwendet wird \citep{manOpen}.
Damit entsteht nach dem Öffnen eine bestehende Zugriffssituation, die unabhängig vom ursprünglichen Pfad weiter über den FD genutzt werden kann.

Für eine Zugriffskontrolle über \texttt{file\_open} ist das wichtig, weil der LSM-Hook beim Öffnen der Datei greift. 
Wenn sich danach entscheidungsrelevante Attribute ändern, wird derselbe bereits geöffnete FD nicht automatisch erneut durch \texttt{file\_open} bewertet.
Eine ASBAC-orientierte Zugriffskontrolle muss deshalb zusätzlich betrachten, wie bestehende FDs überwacht und bei geänderter Entscheidungsgrundlage behandelt werden können.

Linux stellt über \texttt{/proc/<pid>/fd} Informationen über die geöffneten FDs eines Prozesses bereit.
...
Für den Prototyp kann ein Userspace-PEP diese Informationen nutzen, um zu erkennen, welcher Prozess über welchen FD auf welche Datei zugreift.
...
Eine solche nachträgliche Behandlung ist jedoch nicht atomar. Zwischen dem Ermitteln eines File
Descriptors, der Policy-Auswertung und dem späteren Entzugsversuch kann sich der Zustand des
Prozesses verändern. Eine derartige Zustandsänderung zwischen Prüfung und Verwendung wird als
Time-of-check-to-time-of-use-Race-Condition (TOCTTOU) bezeichnet \citep{mitreCwe367}. Linux kann
eine geschlossene File-Descriptor-Nummer erneut vergeben; bei mehrthreadigen Prozessen kann zudem
eine bereits laufende Ein-/Ausgabe (E/A) -Operation auch nach dem Schließen des Descriptors noch abgeschlossen
werden \citep{manClose}.

Weitere Referenzen auf dieselbe geöffnete Dateibeschreibung können unabhängig vom betrachteten
Descriptor bestehen. \texttt{dup()} erzeugt einen zusätzlichen FD für dieselbe
geöffnete Dateibeschreibung, und ein durch \texttt{fork()} erzeugter Kindprozess übernimmt
entsprechende Deskriptorreferenzen \citep{manDup,manFork}. Das Schließen eines einzelnen File
Descriptors beseitigt diese weiteren Referenzen nicht. Ebenso bleibt eine mit \texttt{mmap()}
erzeugte Speicherabbildung nach dem Schließen des zugehörigen FDs bestehen
\citep{manMmap}. Der für den Prototyp verwendete \texttt{ptrace}-Mechanismus ermöglicht zwar die
Steuerung eines Zielprozesses, unterliegt jedoch Berechtigungsprüfungen und arbeitet auf der Ebene
einzelner Threads \citep{manPtrace}. Ein FD-basierter Entzug aus dem Userspace kann deshalb nur als
nachträglicher, bestmöglicher Entzugsversuch verstanden werden.



\subsection{NixOS}

NixOS ist eine Linux-Distribution, deren Systemzustand deklarativ beschrieben wird.
Kernel, Systemdienste, Pakete und weitere Einstellungen werden aus Nix-Konfigurationen
erzeugt~\citep{nixosManual}. Dadurch können System- und Entwicklungsumgebung gemeinsam mit
dem Quellcode dokumentiert werden.

Für die Evaluation wird NixOS als Gastbetriebssystem in einer QEMU/KVM-VM eingesetzt.
