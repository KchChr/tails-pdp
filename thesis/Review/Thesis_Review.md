...
3. **FA-06 verspricht „unmittelbares“ Beenden.** Trotz der inzwischen ereignisbasierten Auslösung bestehen zwischen Erkennung und `ptrace`-Eingriff TOCTTOU- und FD-Reuse-Risiken. Duplizierte oder vererbte Deskriptoren, `mmap`, nicht attachbare Prozesse, Threads und PID 1 werden nicht vollständig erfasst bzw. entzogen. Die Thesis benennt einige, aber nicht alle Konsequenzen.
4. **Die Subjektidentität war nicht ausdrücklich definiert.** Die Nachprüfung zeigt, dass sowohl Ayas `ctx.uid()` als auch das erste `Uid:`-Feld in `/proc/<pid>/status` die Real UID liefern. Die Thesis legt diese Semantik nun ausdrücklich fest und grenzt Effective UID, Filesystem-UID, Credential-Wechsel und User Namespaces ab. Ein Userspace-Regressionstest sichert die Feldauswahl; der privilegierte End-to-End-Vergleich bleibt Teil der Evaluation.
...
**Bewertung: technisch stark, aber mit kritischen Nachweislücken.** Die Beschreibung entspricht dem Code überwiegend bemerkenswert genau. Positiv sind die expliziten Grenzen bei ereignisbasierter Nachbewertung, Generationen, `ptrace` und nun auch der Real-UID-Semantik. Kritisch bleiben der ausstehende Credential-E2E-Nachweis, FD-Reuse, Multi-Threading, `mmap`, Descriptor-Duplikation/-Vererbung, Map-Manipulation und der Ausfall des Userspace-PEP. PDF-Seite 38 ist formal nicht abgabefähig.
