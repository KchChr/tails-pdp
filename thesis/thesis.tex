...
Diese Arbeit übernimmt nicht XACML als Sprache, nutzt aber dieselbe grundsätzliche Trennung: Policies werden außerhalb des Durchsetzungspunktes verwaltet und in eine Form überführt, die zur technischen Zugriffskontrolle verwendet werden kann.
...
Dies ist für den Prototyp wichtig, weil nicht nur der Loader, sondern auch ein Monitor oder Administrationswerkzeug den aktuellen Zustand lesen können muss. 
...
Für den Prototyp kann ein Userspace-Monitor diese Informationen nutzen, um zu erkennen, welcher Prozess über welchen File Descriptor auf welche Datei zugreift. 
Wird eine bestehende Zugriffssituation durch neue Attribute oder Policies unzulässig, kann der Monitor gezielt den betroffenen Zugriff behandeln, ohne notwendigerweise alle Zugriffe desselben Prozesses zu beenden.
...
\begin{description}
    ...
    Der Monitor muss erkennen können, welcher Prozess über welchen File Descriptor auf welche Datei zugreift und ob dieser Zugriff nach der aktuellen Policy- und Attributlage weiterhin zulässig ist.

    \textit{Akzeptanzkriterium:} Öffnet ein Prozess eine Datei zunächst zulässig und wird der Zugriff anschließend durch eine Änderung eines entscheidungsrelevanten Attributs unzulässig, erkennt der Monitor diese Verletzung und kann Prozess-ID, File Descriptor und betroffenen Dateipfad ausgeben.
...
\begin{itemize}
    ...
    Dieser unterteilt sich in das Hauptprogramm, einen Attributloader, einen Monitor und der Policyverwaltung.
    ...
    Der Monitor überwacht Dateizugriffe im Userspace und prüft mögliche Policy-Verletzungen.
...
Zusätzlich müssen weitere Userspace-Komponenten, insbesondere Monitor und Administrationswerkzeug, den aktuellen Zustand lesen können.
...
Dadurch erhalten Loader, Monitor und Administrationswerkzeug einen stabilen Zugriffspfad auf dieselben Kernelobjekte, während das eBPF-Programm die Werte direkt im Kernel auswerten kann.
...
Die Entscheidung lautet daher, die unmittelbare Hook-Entscheidung durch einen Userspace-Monitor zu ergänzen.
Der Monitor liest regelmäßig Informationen über laufende Prozesse und offene File Descriptors aus dem System.
...
Wird eine Verletzung erkannt, kann der Monitor ein Enforcement auslösen.
...
Der Monitor behandelt dagegen bestehende Zugriffssituationen, die nach der ursprünglichen Entscheidung weiterlaufen.
...
Wenn der Monitor eine bestehende Policy-Verletzung erkennt, muss der Prototyp den unzulässig gewordenen Zugriff beenden können.
...
Der Monitor ermittelt Prozess-ID und File-Descriptor-Nummer der verletzenden Zugriffssituation.
...
Für gehashte Attributnamen und Zeichenkettenwerte baut es zusätzlich ein Wörterbuch aus den Policy- und Environment-Dateien auf.
...
Eine weitere Alternative wäre, Diagnose direkt in das Hauptprogramm oder den Monitor einzubauen.
...
Die Diagnose bleibt allerdings teilweise best-effort: Wenn ein Hashwert nicht aus den vorhandenen Policy- oder Environment-Dateien rekonstruiert werden kann, kann das Tool ihn nur als Hash beziehungsweise unbekannten Wert anzeigen.
